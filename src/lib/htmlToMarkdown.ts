...
export function handleSmartPaste(
  ...
  setMarkdownInput: (val: string) => void,
  uploadClipboardImage?: (file: File, index: number, total: number) => Promise<string>
): void {
  ...
  if (imageFiles.length > 0) {
    ...
    const imageTasks = imageFiles.map(async (file, index) => {
      if (uploadClipboardImage) {
        try {
          return await uploadClipboardImage(file, index, imageFiles.length);
        } catch (err) {
          console.error("Clipboard image upload failed, fallback to data URL:", err);
        }
      }
    ...
    Promise.all(imageTasks)
      ...
      .catch((err) => {
        console.error("Clipboard image processing failed:", err);
        alert("粘贴图片失败，请重试");
      });
    return;
  }
  ...
  if (htmlData && htmlData.trim() !== "") {
    ...
    const isMainlyCode =
      (hasPreTag || hasCodeTag) && !htmlData.includes("<p") && !htmlData.includes("<div");
