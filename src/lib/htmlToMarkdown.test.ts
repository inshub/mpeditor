...
describe("insertAtSelection", () => {
  ...
  it("replaces the active selection and moves the caret after the inserted text", () => {
    ...
    let nextValue = "";
    insertAtSelection(textarea, "mpeditor", (value) => {
      nextValue = value;
      textarea.value = value;
    });

    expect(nextValue).toBe("hello mpeditor");

    vi.runAllTimers();

    expect(textarea.selectionStart).toBe("hello mpeditor".length);
    expect(textarea.selectionEnd).toBe("hello mpeditor".length);
  });
});
